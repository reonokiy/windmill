import type { BlobStore } from "../../f/lib/blob.ts";

/** Shared offline backend contract check; never deployed as a Windmill script. */
export async function verifyBlobRoundtrip(storage: BlobStore) {
  const run = `tmp/${crypto.randomUUID()}`;
  const paths = [`${run}/text.txt`, `${run}/data.json`, `${run}/binary.bin`];
  const bytes = new Uint8Array([0, 127, 128, 255]);
  const failures: unknown[] = [];
  try {
    await storage.writeText(paths[0], "Blob 测试 🌍");
    if (await storage.readText(paths[0]) !== "Blob 测试 🌍") throw new Error("Text mismatch");
  ...
  if (failures.length) throw new AggregateError(failures, "Blob test or cleanup failed");
